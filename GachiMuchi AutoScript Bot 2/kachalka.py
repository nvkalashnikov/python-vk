...
def init(access_token):
    session = vk.Session(access_token=access_token)
    api = vk.API(session, v='5.95')
    kachalka_alums(api, kochka)

def kachalka_alums(api, kochka):
    print("Бот запущен.")
    api.messages.send(random_id=random.randint(1,100000),peer_id=2000000025, message="Автокачалка была успешно запущена.")
    while True:
        if kochka:
...
try:
    ...
    api.messages.send(random_id=random.randint(1,100000),peer_id=2000000025, message="Работа автокачалки была аварийно прекращена. Перезапуск...")
    kochka = True
    init(access_token)
